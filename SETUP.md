...
**Known facts:** domain **culturae.world** bought on GoDaddy under **the friend’s own Gmail**;
...
needed — the friend’s own Gmail (the one that bought the domain)
becomes the owner of everything:

1. **Blogger:** signed in as the friend’s Gmail → blogger.com → New Blog →
...
2. **Vercel:** at vercel.com/signup choose **Continue with Email** with
   the friend’s Gmail (it logs in with a code sent to the mail — no password needed).
...
3. **info@culturae.world (the public address):** GoDaddy → your domain → **Email
   ...
   the friend’s Gmail. Incoming mail to info@ then lands in the friend's Gmail.
...
1. console.cloud.google.com (signed in as **the friend’s Gmail**) → create a
...
3. **APIs & Services → OAuth consent screen** → External → fill app name + emails →
   leave it in **Testing** mode and add **the friend’s Gmail** (and yours if you want)
